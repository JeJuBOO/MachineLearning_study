...
%         out_layer = (out_layer-mean(out_layer))./std(out_layer);
        ...
        % o*p / CONV - FOOL - FC
        gradient_FC2 = reshape(hU3' * gradient3,layerDim2,layerDim2,kernelSize2(4),X_num);
        gradient2 = UpSampling(gradient_FC2,poolDim2,'mean',pool_grad2).*ReluGradient(z2);
        
        % m*n / CONV - FOOL - CONV
        gradient_CONV1 = Convolution(gradient2,hU2);
...
%     out_layer = (out_layer-mean(out_layer))./std(out_layer);
    ...
    time
    figure(1)
     plot(e/length(error):e/length(error):e,error);hold on
