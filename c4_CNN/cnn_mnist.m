...
%         out_layer = (out_layer-mean(out_layer))./std(out_layer);
        ...
        U3 = U3 - (lr./((1e-8)+sqrt(r3))).*(dU3/batch+lambda*U3);
        B3 = B3 - (lr./((1e-8)+sqrt(rb3))).*dB3/batch;
        U2 = U2 - (lr./((1e-8)+sqrt(r2))).*(dU2/batch+lambda*U2);
        B2 = B2 - (lr./((1e-8)+sqrt(rb2))).*dB2/batch;
        U1 = U1 - (lr./((1e-8)+sqrt(r1))).*(dU1/batch+lambda*U1);
        B1 = B1 - (lr./((1e-8)+sqrt(rb1))).*dB1/batch;
...
%     z1 = Normalization3(z1);
    layer1 = Relu(z1);% m*n
    pool_layer1 = Pooling(layer1,poolDim1); % m'*n'
...
%     out_layer = (out_layer-mean(out_layer))./std(out_layer);
    ...
    time
    figure(1)
     plot(e/length(error):e/length(error):e,error);
